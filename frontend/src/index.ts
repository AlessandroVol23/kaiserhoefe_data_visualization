  ...
  const currentMilestones = milestones.map((e: any, i: any) => {
    ...
    if (e.birthday && e.birthday.trim() !== "") {
      ...
      const varInit = (date: any) => {
        if (date.match(reg1)) {
          return moment(moment(date).format("YYYY")).format("YYYY,M,D");
        } else if (date.match(reg2)) {
          return moment(moment(date.replace("~", "")).format("YYYY")).format(
            "YYYY,M,D"
          );
      ...
      if (
        ...
        parseInt(moment(birthday).format("YYYY")) >= parseInt(activeYear)
      ) {
        response.push({
          startDate: birthday,
          headline: `* ${e.name}`,
      ...
      if (
        parseInt(moment(deathday).format("YYYY")) <=
          parseInt(activeYear + 50) &&
        parseInt(moment(deathday).format("YYYY")) >= parseInt(activeYear)
      ) {
        response.push({
          startDate: deathday,
          headline: `† ${e.name}`,
