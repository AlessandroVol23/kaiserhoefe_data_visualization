...
const getEnvironment = async () => {
  const env = fetch(`http://${window.location.hostname}:5000/env`, {
    method: "GET"
  })
    .then(response => {
...
const getIncestData = async env => {
  const incestRequest = await fetch(
    `http://${env}:8000/stats/incest_relationsships`,
    {
      method: "GET"
    }
  );
...
const getTimelineData = async env => {
  const timelineRequest = await fetch(
    `http://${env}:8000/stats/person_birth_death_pic`,
    {
      method: "GET"
    }
  );
