...
const updateFirstStoryConclusion = () => {
  ...
  firstStoryConclusion.innerHTML = values3[activeYear()].first_story_conclusion;
  ...
  if (activeGeoJson != activeYearElementJson()) {
    ...
    d3.json(`/data/data.json`, function(error, h2o) {
      ...
      d3.selectAll("path").each(function(d, i) {
        if (d.properties && activeCountries.includes(d.properties.NAME)) {
          d3.select(this).classed("active", true);
        }
      });
    });
...
const createTimeline = year => {
  ...
  const currentMilestones = milestones.map((e, i) => {
    ...
    if (e.birthday && e.birthday.trim() !== "") {
      ...
      if (
        (birthday,
        parseInt(moment(birthday).format("YYYY")) <= parseInt(year) + 50 &&
      ...
      if (
        (deathday,
        parseInt(moment(deathday).format("YYYY")) <= parseInt(year) + 50 &&
